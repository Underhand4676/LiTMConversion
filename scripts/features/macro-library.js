...
const MACRO_NAME = "Probability Processor // MK VI";
const MACRO_FLAG = "managedProbabilityProcessor";
...
async function ensureProbabilityProcessorMacro() {
  ...
  let macro = game.macros.find(
    candidate => candidate.getFlag(MODULE_ID, MACRO_FLAG) === true
  );
  ...
  if (!macro) {
    ...
    console.log(`${MODULE_ID} | Provisioned ${MACRO_NAME}`);
    return macro;
  }
  ...
  if (macro.getFlag(MODULE_ID, MACRO_FLAG) !== true) {
    update[`flags.${MODULE_ID}.${MACRO_FLAG}`] = true;
  }
